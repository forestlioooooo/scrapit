"""
httpx scraper backend.

A faster, async-capable HTTP client backend that can be used 
as an alternative to BeautifulSoup for simple fetching needs.
    ...
    for attempt in range(retries):
        try:
            with httpx.Client(proxies=proxies, timeout=timeout) as client:
                resp = client.get(url, headers=merged_headers, cookies=cookies)
